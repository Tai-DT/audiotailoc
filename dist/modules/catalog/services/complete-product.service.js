...
let CompleteProductService = class CompleteProductService {
    ...
    async searchProducts(query, limit = 20) {
        return {
            items: [{
                    ...
                    images: undefined,
                    categories: { id: 'test-cat', name: 'Test Category', slug: 'test-categories' },
                    brand: 'Test Brand',
                    model: 'Test Model',
                    sku: 'TEST-001',
    ...
    async updateProduct(id, updateProductDto) {
        ...
        const { name, slug, description, shortDescription, priceCents, originalPriceCents, stockQuantity, maxStock, sku, warranty, features, minOrderQuantity, maxOrderQuantity, tags, categoryId, brand, model, weight, dimensions, specifications, images, isActive, featured, metaTitle, metaDescription, metaKeywords, canonicalUrl, } = updateProductDto;
    ...
    async getProductAnalytics() {
        ...
        productsByCategory.forEach(item => {
            const categoriesName = categoriesMap.get(item.categoryId) || 'Unknown';
            productsByCategoryFormatted[categoriesName] = item._count.id;
        });
        return {
            totalProducts,
            activeProducts,
    ...
    mapToProductResponse(product) {
        return {
            ...
            originalPriceCents: product.originalPriceCents ? Number(product.originalPriceCents) : undefined,
