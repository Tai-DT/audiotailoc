...
let PaymentsService = PaymentsService_1 = class PaymentsService {
        ...
        if (intent.provider === 'PAYOS') {
            ...
            try {
                ...
                const signatureString = sortedKeys
                    ...
                    .join('&');
                const sig = crypto_1.default.createHmac('sha256', checksumKey).update(signatureString).digest('hex');
                const headers = {
    ...
    async createMomoPayment(intent, order, returnUrl) {
        try {
            ...
            const ipnUrl = this.config.get('PAYOS_WEBHOOK_URL') || `${this.config.get('API_BASE_URL')}/webhooks/payos`;
            ...
            const momoEndpoint = this.config.get('MOMO_ENDPOINT') || 'https://test-payment.momo.vn/v2/gateway/api/create';
