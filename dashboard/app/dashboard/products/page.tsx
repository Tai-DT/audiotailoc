...
import { Product, Category, ProductsResponse } from "@/types/dashboard"
...
export default function ProductsPage() {
  ...
  const [pageSize] = useState(20) // Optimized page size
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      const response = await apiClient.getProducts({
        ...
        // Add status filter if needed by backend
        // status: statusFilter === "all" ? undefined : statusFilter || undefined
      })

      const data = response.data as unknown as ProductsResponse

      // Handle both structure variations if necessary, but prefer strict type
  ...
  }, [currentPage, pageSize, categoryFilter, searchTerm, toast])
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-red-600">
                {products?.filter(p => getStock(p) === 0).length || 0}
              </div>
              <p className="text-xs text-muted-foreground">
                Cần nhập hàng
              </p>
                ...
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-12">
                      ...
                      >
                        {selectedProducts.size === (products?.length || 0) && (products?.length || 0) > 0 ? (
                      ...
                      <TableRow key={product.id}>
                        ...
                        <TableCell>
                          <div className="flex items-center space-x-2">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-sm font-medium ${getStock(product) > 10 ? 'bg-green-50 text-green-700' :
                              getStock(product) > 0 ? 'bg-yellow-50 text-yellow-700' : 'bg-red-50 text-red-700'
                              }`}>
                              {formatNumber(getStock(product))}
                            </span>
                            {getStock(product) <= 5 && getStock(product) > 0 && (
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  // Call API
                                  await apiClient.updateProduct(product.id, { isActive: newStatus })
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              disabled={checkingDelete || deleting || (deleteCheckResult !== null && !deleteCheckResult.canDelete)}
            >
              {deleting ? 'Đang xóa...' : checkingDelete ? 'Đang kiểm tra...' : 'Xóa'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
