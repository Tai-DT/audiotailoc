...
export default function CustomersPage() {
  ...
  const filteredCustomers = customers.filter(customer => {
    if (searchQuery && customer.name && !customer.name.toLowerCase().includes(searchQuery.toLowerCase()) &&
  ...
  const handleSendEmail = (customer: Customer) => {
    setSelectedCustomer(customer)
    setEmailContent({
      subject: `Ưu đãi đặc biệt dành cho ${customer.name || "khách hàng"}`,
      message: ""
    })
            ...
            <CardContent>
              <div className="flex items-center gap-4">
                ...
                <select
                  ...
                  title="Chọn phân khúc khách hàng"
                >
                    ...
                    {loading ? (
                      ...
                      filteredCustomers.map((customer) => (
                        <tr key={customer.id} className="border-b hover:bg-muted/50">
                          <td className="py-3">
                            <div className="flex items-center gap-3">
                              <Avatar className="h-10 w-10">
                                ...
                                <AvatarFallback>
                                  {customer.name ? customer.name.split(" ").map(n => n[0]).join("") : customer.email[0].toUpperCase()}
                                </AvatarFallback>
                              </Avatar>
                              <div>
                          ...
                          <td className="py-3">
                            <Badge className={getSegmentColor(customer.segment || "inactive")}>
                              {getSegmentLabel(customer.segment || "inactive")}
                            </Badge>
                          </td>
                  ...
                  <CardContent>
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="pt-2 font-bold">
                        {customers.filter(c => (c.loyaltyPoints || 0) < 1000).length} thành viên
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="pt-2 font-bold">
                        {customers.filter(c => (c.loyaltyPoints || 0) >= 5000).length} thành viên
                      </div>
                    </div>
                  </CardContent>
                </Card>
            ...
            <DialogHeader>
              <DialogTitle className="flex items-center gap-3">
                <Avatar className="h-12 w-12">
                  ...
                  <AvatarFallback>
                    {selectedCustomer.name ? selectedCustomer.name.split(" ").map(n => n[0]).join("") : selectedCustomer.email[0].toUpperCase()}
                  </AvatarFallback>
                </Avatar>
                <div>
                  <h3 className="text-xl font-bold">{selectedCustomer.name || "Unknown Customer"}</h3>
                  <Badge className={getSegmentColor(selectedCustomer.segment || "inactive")}>
                    {getSegmentLabel(selectedCustomer.segment || "inactive")}
                  </Badge>
                </div>
              </DialogTitle>
            </DialogHeader>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm">
                      Đăng ký: {selectedCustomer.registeredAt ? format(new Date(selectedCustomer.registeredAt), "dd/MM/yyyy", { locale: vi }) : "N/A"}
                    </span>
                  </div>
                </div>
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm">
                      TB: {(selectedCustomer.averageOrderValue || 0).toLocaleString('vi-VN')}đ/đơn
                    </span>
                  </div>
                </div>
              </div>
