...
# Script để khởi động Backend, Frontend và Dashboard
...
echo -e "${GREEN}🚀 Khởi động Backend, Frontend và Dashboard...${NC}"
echo ""
...
if lsof -ti:3001 > /dev/null 2>&1; then
    echo -e "${YELLOW}⚠️  Dashboard đã chạy trên port 3001${NC}"
else
    echo -e "${BLUE}📊 Khởi động Dashboard (port 3001)...${NC}"
    cd dashboard
...
if curl -s http://localhost:3001 > /dev/null 2>&1; then
    echo -e "${BLUE}✅ Dashboard đang chạy: http://localhost:3001${NC}"
else
    echo -e "${YELLOW}⏳ Dashboard đang khởi động...${NC}"
    echo "   Xem log: tail -f /tmp/dashboard.log"
fi
...
echo "   kill \$(lsof -ti:3001)  # Dashboard"
