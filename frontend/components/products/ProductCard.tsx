				...
				{showAddToCart && (
					<div className="flex items-center gap-2">
						...
						<button
							...
							onClick={(e) => {
								e.preventDefault();
								addCompare({ id: product.id, slug: product.slug, name: product.name, imageUrl: product.imageUrl, priceCents: product.priceCents, categorySlug: product.category?.slug, specs: (product as any).specs });
							}}
							className={`px-3 py-2 text-sm rounded-md border ${isInCompare(product.id) ? 'bg-blue-50 text-blue-700 border-blue-200' : 'text-gray-700 border-gray-200 hover:bg-gray-50'}`}
						>
							So sánh
						</button>
					</div>
