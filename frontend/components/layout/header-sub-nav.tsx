        ...
        <div className="container mx-auto px-4 lg:px-6 h-full flex items-center justify-start md:justify-center gap-3 md:gap-5 overflow-x-auto scrollbar-hide">
            {SUB_CATEGORIES.map((cat) => (
                ...
                >
                    ...
                    <span className="text-[9px] md:text-[11px] font-black uppercase tracking-[0.2em] text-foreground/70 group-hover:text-primary transition-colors font-display italic">
                        {cat.name}
                    </span>
                    <div className="absolute bottom-0 left-0 w-full h-[1.5px] bg-primary scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-center" />
                </Link>
            ))}
        </div>
    );
