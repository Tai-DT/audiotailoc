...
export const useTopViewedProducts = (limit = 10) => {
  return useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await apiClient.get('/catalog/products/top-viewed', {
          params: { limit },
        });
...
export const useFeaturedServices = (limit = 4) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const featuredResponse = await apiClient.get('/services', {
        params: {
          isFeatured: true,
          isActive: true,
          page: 1,
      ...
      const featuredData = handleApiResponse<{ services?: Service[]; items?: Service[]; total?: number }>(featuredResponse);
      const featuredServices = featuredData.services || featuredData.items || [];

      // If no featured services, fallback to regular active services
      if (featuredServices.length === 0) {
        const fallbackResponse = await apiClient.get('/services', {
          params: {
            isActive: true,
            page: 1,
            pageSize: limit,
        ...
        const fallbackData = handleApiResponse<{ services?: Service[]; items?: Service[]; total?: number }>(fallbackResponse);
        return fallbackData.services || fallbackData.items || [];
      }
...
export const useCreateServiceBooking = () => {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      const token = authStorage.getAccessToken();
      const hasValidToken = !!(token && token.trim().length > 0 && token !== 'null');
...
export const useProductReviews = (productId: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      // Use /reviews endpoint with productId query param
