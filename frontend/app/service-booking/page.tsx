...
export default function ServiceBookingPage() {
 ...
 const watchedAddress = watch('customerAddress');
 ...
 const description = selectedPlace.description || selectedPlace.name || '';
 if (watchedAddress !== description) {
 setSelectedPlace(null);
 setBookingCoordinates(null);
 }
 }, [watchedAddress, selectedPlace]);

 const handleSelectSuggestion = async (suggestion: AddressSuggestion) => {
 const description = suggestion.description || suggestion.name || '';
 const placeId = suggestion.place_id || suggestion.placeId;

 setValue('customerAddress', description, { shouldDirty: true, shouldValidate: true });
 ...
 />
 {isSearchingAddress && (
 <p className="text-xs text-muted-foreground mt-1">Đang tìm kiếm địa chỉ...</p>
 )}
 ...
 <p className="text-muted-foreground mb-4">{hotlineDisplay}</p>
 <Button variant="outline" size="sm" asChild>
 <Link href={`tel:${hotlineNumber}`}>Gọi ngay</Link>
 </Button>
 </CardContent>
 </Card>
