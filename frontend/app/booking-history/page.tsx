...
export default function BookingHistoryPage() {
 ...
 // Backend ServiceBooking has 'address' field? Let's check types.ts again.  // It seems ServiceBooking in types.ts DOES NOT have address.  // But checking Prisma schema: model service_bookings has 'address', 'coordinates'.
 ...
 case 'ASSIGNED':
 return <Badge className="bg-indigo-500">Đã phân công</Badge>;
 case 'IN_PROGRESS':
 return <Badge className="bg-yellow-500">Đang thực hiện</Badge>;
 case 'RESCHEDULED':
 return <Badge className="bg-purple-500">Đã đổi lịch</Badge>;
 case 'PENDING':
 ...
 {booking.scheduledTime || (booking.scheduledAt ? format(new Date(booking.scheduledAt), 'HH:mm', { locale: vi }) : '--:--')}
 </p>
 </div>
 </div>
 </TableCell>
 ...
 {selectedBooking.scheduledAt ? format(new Date(selectedBooking.scheduledAt), 'dd/MM/yyyy', { locale: vi }) : 'Chưa xác định'}{' '}
