...
export async function POST(req: NextRequest) {
	...
	if (!res.ok) return NextResponse.json({ error: 'Đăng ký thất bại' }, { status: 400 });
	// Immediately login after register for better UX
	...
	if (!login.ok) return NextResponse.json({ ok: true });
	const data = (await login.json()) as { accessToken: string; refreshToken: string };
