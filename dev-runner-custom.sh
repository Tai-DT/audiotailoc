...
start_service() {
    ...
    local log_file="../$service_name-custom.log"
    ...
    echo $pid > "../$service_name-custom.pid"
...
stop_services() {
    ...
    # Stop frontend
    if [ -f "frontend/dev-custom.pid" ]; then
        local pid=$(cat frontend/dev-custom.pid)
        ...
        rm -f frontend/dev-custom.pid
    fi
    kill_port 3000 "Frontend"

    # Stop backend
    if [ -f "backend/dev-custom.pid" ]; then
        local pid=$(cat backend/dev-custom.pid)
        if kill -0 $pid 2>/dev/null; then
            kill -TERM -- -$pid 2>/dev/null || kill -TERM $pid 2>/dev/null || true
            sleep 1
            if kill -0 $pid 2>/dev/null; then
        ...
        rm -f backend/dev-custom.pid
    fi
    kill_port "$backend_port" "Backend"

    # Stop dashboard
    if [ -f "dashboard/dev-custom.pid" ]; then
        local pid=$(cat dashboard/dev-custom.pid)
        if kill -0 $pid 2>/dev/null; then
            kill -TERM -- -$pid 2>/dev/null || kill -TERM $pid 2>/dev/null || true
            sleep 1
            if kill -0 $pid 2>/dev/null; then
...
clean_files() {
    ...
    rm -f frontend/dev-custom.log frontend/dev-custom.pid
    rm -f backend/dev-custom.log backend/dev-custom.pid
    rm -f dashboard/dev-custom.log dashboard/dev-custom.pid
...
case "${1:-start}" in
    "start")
        ...
        if [ -f "frontend/dev-custom.pid" ] || [ -f "backend/dev-custom.pid" ] || [ -f "dashboard/dev-custom.pid" ]; then
