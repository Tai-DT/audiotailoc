/**
 * Script to delete old folders in Cloudinary
 * 
 * ⚠️ WARNING: This action is IRREVERSIBLE!
 * All assets in the specified folders will be permanently deleted.
 * 
 * Usage:
 *   node scripts/delete-old-folders.js
 * 
 * Or with confirmation:
 *   node scripts/delete-old-folders.js --confirm
 */

require('dotenv').config({ path: require('path').resolve(__dirname, '../.env') });
const cloudinary = require('cloudinary').v2;

// List of old folders to delete (update this list with actual folder names)
const OLD_FOLDERS_TO_DELETE = [
  // Add your 18 folder names here, for example:
  // 'old-uploads',
  // 'legacy-images',
  // 'temp-files',
  // ... (18 folders total)
...
async function deleteOldFolders(confirm = false) {
  ...
  if (OLD_FOLDERS_TO_DELETE.length === 0) {
    console.error('❌ No folders specified in OLD_FOLDERS_TO_DELETE array!');
    console.error('Please edit this script and add the folder names to delete.');
    process.exit(1);
  }
  ...
  if (!confirm) {
    ...
    OLD_FOLDERS_TO_DELETE.forEach((folder, index) => {
      console.log(`   ${index + 1}. ${folder}`);
    });
    console.log(`\n⚠️  Total folders: ${OLD_FOLDERS_TO_DELETE.length}`);
    console.log('⚠️  This action is IRREVERSIBLE!');
    console.log('\nTo proceed, run with --confirm flag:');
    console.log('  node scripts/delete-old-folders.js --confirm\n');
    process.exit(0);
  }

  console.log('⚠️  CONFIRMED: Starting deletion of old folders...\n');
  console.log(`📁 Folders to delete: ${OLD_FOLDERS_TO_DELETE.length}\n`);
  ...
  try {
    // Process each folder
    for (const folder of OLD_FOLDERS_TO_DELETE) {
      ...
      do {
        // Search for all assets that start with this folder name
        ...
        try {
          ...
          // If search fails, try alternative method
          console.log(`   ⚠️  Search failed, trying alternative method...`);
          try {
            result = await cloudinary.search
            ...
            console.log(`   ℹ️  No assets found or folder doesn't exist: ${folder}`);
            break;
          }
        }
    ...
    console.log(`📁 Folders processed: ${OLD_FOLDERS_TO_DELETE.length}\n`);
