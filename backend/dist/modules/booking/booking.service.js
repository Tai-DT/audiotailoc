...
let BookingService = class BookingService {
    ...
    async getBookingStats(params) {
        ...
        const [totalBookings, pendingBookings, confirmedBookings, inProgressBookings, completedBookings, cancelledBookings, totalRevenue,] = await Promise.all([
            this.prisma.serviceBooking.count({ where }),
            this.prisma.serviceBooking.count({ where: { ...where, status: enums_1.ServiceBookingStatus.PENDING } }),
            this.prisma.serviceBooking.count({ where: { ...where, status: enums_1.ServiceBookingStatus.CONFIRMED } }),
            this.prisma.serviceBooking.count({ where: { ...where, status: enums_1.ServiceBookingStatus.IN_PROGRESS } }),
            this.prisma.serviceBooking.count({ where: { ...where, status: enums_1.ServiceBookingStatus.COMPLETED } }),
            this.prisma.serviceBooking.count({ where: { ...where, status: enums_1.ServiceBookingStatus.CANCELLED } }),
