...
export class AuthService {
  ...
  async login(dto: { email: string; password: string; rememberMe?: boolean }) {
    ...
    if ((user as any).role === 'DISABLED') {
      this.securityService.recordLoginAttempt(dto.email, false);
      throw new Error('Account disabled');
    }

    console.log('🔐 Comparing passwords...');
    // Defensive password comparison to support legacy plaintext passwords
    let ok = false;
    try {
      const stored = user.password ?? '';
      const looksLikeBcrypt = typeof stored === 'string' && (stored.startsWith('$2a$') || stored.startsWith('$2b$') || stored.startsWith('$2y$'));

      if (looksLikeBcrypt) {
        ok = await bcrypt.compare(dto.password, stored);
        console.log('🔐 Password comparison (bcrypt) result:', ok);
      } else {
        // Legacy stored plaintext password (or other format). Compare directly.
        console.log('⚠️ Stored password does not look like bcrypt hash; attempting direct comparison');
        ok = dto.password === stored;
        console.log('🔐 Password comparison (plaintext) result:', ok);

        if (ok) {
          // Re-hash the plaintext password using bcrypt and update the user record to be secure.
          try {
            const newHash = await bcrypt.hash(dto.password, 12);
            await this.users.updatePassword(user.id, newHash);
            console.log('🔁 Re-hashed legacy plaintext password and updated user record');
          } catch (updateErr) {
            console.error('❌ Failed to update hashed password for legacy account:', updateErr?.message || updateErr);
          }
        }
      }
    } catch (err) {
      console.error('❌ Error during password comparison:', err?.message || err);
      ok = false;
    }
