...
export class AnalyticsService {
  ...
  private serializeFilters(filters: AnalyticsFilters): JsonValue {
    const cloned = { ...filters } as Record<string, unknown>;
    if (cloned.startDate instanceof Date) {
      cloned.startDate = cloned.startDate.toISOString();
    }
    if (cloned.endDate instanceof Date) {
      cloned.endDate = cloned.endDate.toISOString();
    }
    return cloned as JsonValue;
  }

  private buildDownloadUrl(filename: string): string {
    return `/api/v1/analytics/download/${filename}`;
  }

  private describePeriod(filters: AnalyticsFilters): string {
    if (!filters.startDate && !filters.endDate) return 'unspecified';
    const start = filters.startDate instanceof Date ? filters.startDate : filters.startDate ? new Date(filters.startDate) : null;
    const end = filters.endDate instanceof Date ? filters.endDate : filters.endDate ? new Date(filters.endDate) : null;

    if (start && end) {
      const sameDay = start.toDateString() === end.toDateString();
      if (sameDay) {
        return `Ngày ${start.toLocaleDateString('vi-VN')}`;
      }
