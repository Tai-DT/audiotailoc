...
export class ProjectsService {
  ...
  async create(data: any) {
    ...
    if (!data.userId) {
      ...
      if (!fallbackUser) {
        throw new BadRequestException('User is required to create a project');
      }
    ...
    if (data.teamSize && typeof data.teamSize === 'string') {
      const parsed = Number.parseInt(data.teamSize, 10);
      data.teamSize = Number.isNaN(parsed) ? null : parsed;
    }

    const dateFields: Array<'startDate' | 'endDate' | 'projectDate' | 'completionDate'> = [
      'startDate',
      'endDate',
      'projectDate',
      'completionDate',
    ...
    for (const field of dateFields) {
      if (data[field]) {
        ...
        data[field] = Number.isNaN(dateValue.getTime()) ? null : dateValue;
      }
    }
