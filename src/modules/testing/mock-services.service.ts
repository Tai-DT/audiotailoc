...
export class MockServicesService {
  ...
  // Mock notification service disabled placeholder
  async mockNotificationService(operation: string, data: any = {}) {
    if (!this.mocksEnabled) {
      throw new Error('Mock services are disabled in production');
    }

    this.logger.log(`Mock Notification Service disabled - ${operation}`, data);
    return {
      success: false,
      error: 'Notification system is disabled in this environment',
    };
  }
