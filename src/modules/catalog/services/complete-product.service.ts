...
export class CompleteProductService {
  ...
  async updateProduct(id: string, updateProductDto: UpdateProductDto): Promise<ProductResponseDto> {
    ...
    // Check categories if updating categoryId
  ...
  async bulkUpdateProducts(bulkUpdateDto: BulkUpdateProductsDto): Promise<{ updated: number }> {
    ...
    // Check if categories exists if updating categories
    if (categoryId) {
      const categories = await this.prisma.categories.findUnique({
        where: { id: categoryId },
      });
  ...
  async getProductAnalytics(): Promise<ProductAnalyticsDto> {
    ...
    // Get categories names for productsByCategory
        ...
        headers.forEach((header, index) => {
          ...
          switch (header.toLowerCase()) {
            ...
            case 'sku':
              ...
              break;
            case 'categories':
              // Would need to map categories name to ID
              break;
            case 'brand':
