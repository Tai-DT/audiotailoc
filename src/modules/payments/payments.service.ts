...
export class PaymentsService {
  ...
  private async buildRedirectUrl(
    ...
    if (intent.provider === 'PAYOS') {
      ...
      try {
        ...
        // Sort keys alphabetically and create query string: key1=value1&key2=value2...
        // Handle nested objects/arrays according to PayOS docs
        const sortedKeys = Object.keys(payload).sort();
        const signatureString = sortedKeys
          .map(key => {
            let value = payload[key];
            // Handle arrays - sort and stringify
            if (Array.isArray(value)) {
              // Sort array items by their keys if they are objects
              const sortedArray = value.map(item => {
                if (typeof item === 'object' && item !== null) {
                  const sortedItem: any = {};
                  Object.keys(item).sort().forEach(k => sortedItem[k] = item[k]);
                  return sortedItem;
                }
                return item;
              });
              value = JSON.stringify(sortedArray);
            }
        ...
        const sig = crypto.createHmac('sha256', checksumKey).update(signatureString).digest('hex');
  ...
  private async createMomoPayment(
    ...
    try {
      ...
      const ipnUrl = this.config.get<string>('PAYOS_WEBHOOK_URL') || `${this.config.get('API_BASE_URL')}/webhooks/payos`;
      const amount = intent.amountCents;
      const requestType = 'payWithATM';
      const extraData = JSON.stringify({
        orderNo: order.orderNo,
        amountCents: order.totalCents ?? intent.amountCents
      });
  ...
  async markPaid(provider: 'VNPAY' | 'MOMO' | 'PAYOS', txnRef: string, transactionId?: string) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await tx.orders.update({ where: { id: intent.orderId }, data: { status: 'PAID' } });
  ...
  async createRefund(paymentId: string, amountCents?: number, reason?: string) {
    ...
    try {
      ...
      if (payment.orders.userId) {
        // TODO: Implement notification system
        this.logger.log(`Refund notification sent to user ${payment.orders.userId}`);
      }
